...
public class VehicleController {
    ...
    @Get("/find-vehicle-category/{type}")
    public MutableHttpResponse<List<VehicleDTO>> getVehicleByCategory(@PathVariable String type) {
        return HttpResponse.ok().body(vehicleService.findVehicleByType(type));
    }

    @Post("/register-vehicle")
    public HttpResponse<?> createVehicle(@Body @Valid VehicleUpdateDTO vehicle) {
        vehicleService.createVehicle(vehicle);
        return HttpResponse.created("Registered with successfully");
    }

    @Put("/update-vehicle/{id}")
    public HttpResponse<?> updateVehicle(VehicleUpdateDTO vehicle, @PathVariable Long id){
